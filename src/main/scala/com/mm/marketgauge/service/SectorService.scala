...
trait SectorService extends LogHelper {
  ...
  def downloadSectorData(sectorId: Int): Option[Sector] = {
    try {
      Some(_extractSectorData(dataDownloader.downloadFromURL(sectorDataUrl.replace("<sectorId>", sectorId.toString)),
        sectorId))
    } catch {
      case e: java.lang.Exception => {
        logger.info(s"Exception while fetching $sectorId \n$e")
        None
      }
